...
export default function InvestigatePage() {
  ...
  return (
    <div className="mx-auto max-w-[900px] space-y-6">
      ...
      {showBadge&&<div className="inline-flex items-center gap-1.5 rounded-full border border-[var(--nn-border)] bg-[var(--nn-surface)] px-3 py-1 font-mono text-[0.72rem] text-[var(--nn-text-dim)]"><Loader2 size={12} className="animate-spin"/>fireworks · kimi-k2p5</div>}
      {cachedAt&&<div className="inline-flex items-center gap-1.5 rounded-full border border-[var(--nn-slate)] bg-[var(--nn-slate-dim)] px-3 py-1 font-mono text-[0.72rem] text-[var(--nn-slate)]"><Clock size={12}/>Viewing cached analysis from {new Date(cachedAt).toLocaleString()}</div>}
      ...
      {err&&<div className="rounded-[14px] border border-[var(--nn-red)] bg-[var(--nn-red)]/10 px-5 py-4"><div className="flex items-start gap-3"><AlertTriangle size={18} className="mt-0.5 shrink-0 text-[var(--nn-red)]"/><div><p className="font-sans text-[0.84rem] font-semibold text-[var(--nn-red)]">{err}</p><button type="button" onClick={()=>{setErr(null);setQ(qRan);}} className="mt-2 rounded-full border border-[var(--nn-red)] px-3 py-1 font-sans text-[0.74rem] text-[var(--nn-red)] hover:bg-[var(--nn-red)]/10">Retry</button></div><button onClick={()=>setErr(null)} className="ml-auto shrink-0 rounded p-1 text-[var(--nn-text-dim)] hover:text-[var(--nn-text)]"><X size={14}/></button></div></div>}
      ...
      {history.length>0&&!hasArts&&!running&&<div className="rounded-[14px] border border-[var(--nn-border)] bg-[var(--nn-surface)] p-6"><div className="mb-3 flex items-center justify-between"><h2 className="font-heading text-[1rem] font-bold text-[var(--nn-text)]">Recent analyses</h2><button onClick={clearH} className="font-mono text-[0.7rem] text-[var(--nn-text-dim)] hover:text-[var(--nn-text)]">Clear all</button></div><div className="space-y-2">{history.slice(-3).reverse().map((h,i)=>(<button key={i} onClick={()=>loadCached(h)} className="w-full rounded-[10px] border border-[var(--nn-border)] bg-[var(--nn-surface2)] px-4 py-3 text-left hover:border-[var(--nn-navy)] transition-colors"><p className="font-sans text-[0.82rem] text-[var(--nn-text)]">{h.query}</p><p className="mt-0.5 font-mono text-[0.68rem] text-[var(--nn-text-dim)]">{rtime(h.timestamp)} · {h.articles.length} articles · {h.canonicalClaims.length} claims</p></button>))}</div></div>}

      {!hasArts&&!running&&!err&&history.length===0&&<div className="rounded-[14px] border border-[var(--nn-border)] bg-[var(--nn-surface)] p-8 text-center"><Search size={28} className="mx-auto mb-3 text-[var(--nn-text-dim)]"/><p className="font-sans text-[0.84rem] text-[var(--nn-text-dim)]">Enter a subject query to analyze coverage from our curated source panel in real time.</p><p className="mt-1 font-sans text-[0.74rem] text-[var(--nn-text-dim)]">The pipeline searches, fetches, embeds, extracts claims, and computes consensus — all live.</p></div>}
    </div>
  );
}
