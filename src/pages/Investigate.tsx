...
export default function InvestigatePage() {
  ...
  const [artCount,setArtCount]=useState(0); const [succCount,setSuccCount]=useState(0);
  const [matchedCount,setMatchedCount]=useState(0);
  const [defaultThresholds,setDefaultThresholds]=useState<Record<string,number>|null>(null);
  ...
  const runQ=useCallback(async(t0:string)=>{const t=t0.trim();if(!t||running)return;
    ...
    setArtCount(0);setSuccCount(0);setMatchedCount(0);setDefaultThresholds(null);setMinCorr(2);
    ...
    try{
      ...
      for(const ln of lines){
        ...
        case"consensus_result":upStage("consensus","done");timesRef.current.consensus=now-(timesRef.current.consensus||now);co0=(d.per_claim||[]);setCons(co0);setTh(65);setDefaultThresholds(d.thresholds||null);setMinCorr(d.min_corroboration??2);break;
        case"done":tms=d.total_ms;setT(tms);setArtCount(d.articles_analyzed||0);setSuccCount(d.successful_articles||0);setMatchedCount(d.claims_matched||0);for(const s of STAGES)upStage(s,"done");break;
  ...
  const crossSourceMatches = useMemo(() => (_canons||[]).filter(c => c.source_count > 1), [_canons]);
