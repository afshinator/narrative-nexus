...
function ScraperStatus() {
  ...
  // Treat unknown/error state as paused — safer default given the read-only guard.
  const isRunning = status?.running === true;

  return (
    <div
      className="animate-fade-up mt-7 rounded-[14px] border border-(--nn-border) bg-(--nn-surface) p-5"
      style={{ "--i": 1.5 } as React.CSSProperties}
    >
      <p className="font-sans text-[0.82rem] leading-relaxed text-(--nn-text-dim)">
        {isRunning ? (
          <>
            Scraper is running: polling RSS feeds, ingesting new articles, and
            rescanning on a schedule. Manage collection in Settings.
          </>
        ) : (
          <>
            Scraper paused. Toggle in Settings to start collecting live from RSS feeds.
          </>
        )}
      </p>
    </div>
