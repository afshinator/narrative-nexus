...
> *"Narrative Alpha tracks consensus reality, not truth."*

The walkthrough defines the five terms the system uses:

- **Consensus reality** — The version of events agreed upon by the majority of the panel's consensus pool at a given threshold. Not "the truth" — just what most sources say.
...
### 3.2 Landing — The Leaderboard

The user lands on the leaderboard. They see a table of news sources with six columns of metrics, each colored green/amber/red. No composite score — the user decides which metric matters to them.
...
A scatter plot next to the table shows the same data visually: origination rate on the X axis, validation rate on the Y axis. The four quadrants map directly to the four archetype labels. Users can filter by topic vertical (Geopolitics, Economics, Technology) and sort by any column.
...
- **Outlier claim waterfall** — A brDots higher on the Y axis publish claims the panel later validates. Dots further right originate more unique claims.eakdown of where this source's outlier claims ended up. How many became consensus reality? How many fizzled out? How many were copies of another source's scoop?
...
### 3.4 The Story Level — Cluster Forensic Report
...
### 3.5 The TimelineDots higher on the Y axis publish claims the panel later validates. Dots further right originate more unique claims.
...
The default theme is a dark terminal / forensic intelligence console: black backgrounds (#0a0a0f), green positive indicators (#00ff88), amber warnings (#ffaa00), red alerts (#ff4444). Monospace for data values, sans-serif for prose. Animations are purposeful — particles show real data flow, never decorative.

The header includes a light/dark toggle. Light mode inverts the palette for daytime readability while preserving the polarity colors (green/amber/red encoding favorable/middling/unfavorable). The user's choice persists in localStorage and applies on every page load. The font scale slider in Settings is independent of theme — both settings compose.

Every page carries the footer: *"Narrative Alpha tracks consensus reality, not truth."*
