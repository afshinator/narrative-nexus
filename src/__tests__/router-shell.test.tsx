...
describe("Router Shell — Slice 0", () => {
  ...
  it("renders nav bar with 8 navigation links", () => {
    ...
    expect(screen.getByRole("link", { name: /source profile/i })).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /cluster report/i })).toBeInTheDocument();
  ...
  it("active nav link has aria-current=\"page\"", () => {
    render(<App />);
    const sourcesLink = screen.getByRole("link", { name: /^sources$/i, current: "page" });
