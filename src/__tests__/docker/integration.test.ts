...
function composePsJson() {
	const out = execSync(`docker compose -f ${COMPOSE_FILE} ps --format json`, {
		encoding: "utf8",
	});
	return out
		.trim()
...
function containerInspect(service: string) {
	const id = execSync(`docker compose -f ${COMPOSE_FILE} ps -q ${service}`, {
		encoding: "utf8",
	}).trim();
...
describeIf("Docker Compose — integration", () => {
	beforeAll(() => {
		...
		try {
			...
			console.warn(
				"Docker compose up failed — skipping integration tests:",
				(e as Error).message,
			);
		}
	}, 90_000);
