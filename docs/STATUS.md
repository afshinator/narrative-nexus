...
**Last updated:** 2026-07-06 (post-D3)
**Phase:** Docker gate — D2 build failure diagnosed, D3 fix committed locally. Push blocked (SSH).
...
D2 complete: multi-stage Dockerfile.app (self-sufficient frontend build), demo DB baked into image, volume footgun resolved, scheduler confirmed OFF, human runbook delivered. D3: better-sqlite3 removed from frontend devDependencies — not used by build path, native module breaks npm ci in node:20-slim. Commit ac89bc5 ready. Push blocked by SSH. Next: human pushes D3, rebuilds from clean clone.
