...
**Last updated:** 2026-07-09 (post-UX59)
**Phase:** UX59 — Docker chapter closeout (documentation only). UX57+UX58 verified on host: single-container Docker judge-runnable (smoke.sh ALL PASS, /api/stats 358/37, browser pages populated, egress OK). Keyless scraper functional (358→1855 disposable copy, host DB intact). NN_DISABLE_SCRAPER=1 verified: status disabled:true, start returns 403, Settings button disabled with amber text. Deferred: HF Spaces deployment (after deck/video). FP: 378/10/358/17/13653.
...
| 31 | Shipped below font floor (design law 2) | UX54 shipped footer stats line at 0.7rem (11.2px), below the 0.75rem (12px) font floor. Not caught in review — stat line is in a footer far from other text, making the violation easy to miss. Fixed in UX55: raised to text-[0.75rem]. |
